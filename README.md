...
**Финансы:**
- **Платежи** — поступления и расходы; одна оплата на один счёт (MVP); auto-пересчёт статуса
- **Задолженность** — частичная оплата → PARTIALLY_PAID; полная → PAID + paidAt; удаление платежа откатывает статус
- **Акты сверки** — взаимные расчёты с контрагентом за период с frozen-snapshot и PDF
...
7. Открыть СЧ-0001/2026 → кнопки «Акт» / «УПД» / «ТОРГ-12» — создаст наследник с теми же позициями.
8. **Финансы → Акты сверки → Сформировать акт** → выбрать организацию, контрагента, период → видеть live-preview → сохранить.
9. На любом документе — «Превью» (PDF в модалке) или «Скачать PDF».
10. **Дашборд:** виджеты выручки, топ должников, ближайшие платежи; ниже — **калькулятор НДС для УСН 2026**.
11. **Помощник → Импорт:** скачать шаблон контрагентов, отредактировать, загрузить → dry-run → applied.
12. В профиле (нижняя кнопка sidebar) — переключить **тёмную тему**.
...
- **Multi-allocation платежа:** один платёж = одна аллокация на один счёт. Распределение одного платежа на N счетов через UI пока не поддерживается.
...
- **Frontend chunk warning:** `dist/assets/index-*.js` > 500 KB (gzip 193 KB — допустимо для SPA с 18 страницами). Будущее: code-splitting через `manualChunks`.
