...
- **Покрытие routes (Sprint 9A + 9B):** RBAC и cross-org 404-privacy теперь применяются на всех data-routes — `organizations`, `counterparties`, `nomenclature`, `invoices`, `payments`, `files`, `bank-import`, `ai` (Sprint 9A) + `acts`, `upds`, `waybills`, `contracts`, `reconciliations`, `contract-templates` (Sprint 9B). Шаблоны договоров — настройки организации (`print:settings`, ADMIN+); все остальные данные — `data:read` (VIEWER+) / `data:write` (ACCOUNTANT+).
- **Известный долг:** `Counterparty` / `Nomenclature` / `ContractTemplate` пока без колонки `organizationId` — read фильтруется через `getAccessibleUserIds` (по userId владельца каждой org). Это compatible-compromise; полноценная per-org миграция справочников вынесена в будущий спринт.
- **Platform admin (Sprint 9C):** глобальная `User.role = ADMIN` (отдельно от `OrganizationMember.role`) даёт **bypass всех org-чеков** — пользователь видит каждую организацию как синтетический OWNER, проходит любой permission gate (`ai:settings`, `members:*`, `data:write`, `print:settings`, ...). Полезно для оператора платформы / deployer'а. Бизнес-инварианты (last-owner guard, status enums) НЕ обходятся. Назначается напрямую в БД: `UPDATE "User" SET role='ADMIN' WHERE email='...';` — UI для управления глобальной ролью пока нет.

**Системная админка (Sprint 10):** страница `/admin/system` (видна только при `User.role=ADMIN`). 4 таба настроек интеграций:
- **DaData** — `token` + `secret` + `baseUrl`. Включает поиск контрагентов по ИНН / адресный suggest. Приоритет: DB-настройка → fallback на env `DADATA_API_KEY`.
- **AI Provider (system default)** — `providerName` / `baseUrl` / `apiKey` / `defaultModel`. Org-level `AiSettings` (на пользователя) пока сохраняет приоритет; system-default войдёт как fallback в следующем шаге.
- **SMTP** — `host` / `port` / `username` / `password` / `fromEmail` / TLS-флаги. Кнопка «Отправить тестовое письмо». Используется будущими invite-email уведомлениями (пока только настройки + test).
- **App** — `publicUrl` / `supportEmail` / `appName`. Метаданные платформы.

Секреты шифруются AES-256-GCM (`lib/secrets.ts`, ключ `APP_ENCRYPTION_KEY` с fallback на `JWT_SECRET`). API никогда не возвращает plaintext — только `secretPresent: bool` и `secretMasked: "ab••••cdef"`. Каждое изменение пишется в `SystemAuditLog` (имена изменённых полей, имена rotated-ключей — без значений).
