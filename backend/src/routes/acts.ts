...
const baseFields = {
  ...
  date: dateString,
  periodStart: dateString.optional().nullable(),
  periodEnd: dateString.optional().nullable(),
...
export async function actsRoutes(app: FastifyInstance) {
  ...
  app.post("/", async (request, reply) => {
    ...
    try {
      const created = await prisma.$transaction(async (tx) => {
        const number = data.number ?? (await nextDocumentNumber(tx, userId, data.organizationId, "ACT", year));
        ...
        await tx.documentItem.createMany({
          data: prepared.map((it) => itemCreateData(it, userId, "ACT", act.id)),
        });
        return act;
      });
      const full = await prisma.act.findUnique({ where: { id: created.id }, include: { items: { orderBy: { sortOrder: "asc" } } } });
      return reply.code(201).send(full);
    } catch (err) {
      if (err instanceof Prisma.PrismaClientKnownRequestError && err.code === "P2002") {
        return reply.code(409).send({ error: "Conflict", message: "Акт с таким номером уже существует" });
      }
      throw err;
    }
  });
  ...
  app.patch("/:id", async (request, reply) => {
    ...
    if (!parsed.success) return reply.code(400).send({ error: "ValidationError", details: parsed.error.flatten() });
    const userId = request.user.sub;
    const existing = await prisma.act.findFirst({ where: { id, userId } });
    if (!existing) return reply.code(404).send({ error: "NotFound" });
    if (isDocStatusLocked(existing.status)) {
      return reply.code(409).send({ error: "Locked", message: `Акт в статусе ${existing.status} нельзя редактировать` });
    }
    const data = parsed.data;
    ...
    try {
      const updated = await prisma.$transaction(async (tx) => {
        ...
        if (data.items) {
          ...
          await tx.documentItem.deleteMany({ where: { actId: id } });
          await tx.documentItem.createMany({ data: prepared.map((it) => itemCreateData(it, userId, "ACT", id)) });
  ...
  app.delete("/:id", async (request, reply) => {
    const { id } = z.object({ id: z.string().uuid() }).parse(request.params);
    const existing = await prisma.act.findFirst({ where: { id, userId: request.user.sub } });
    if (!existing) return reply.code(404).send({ error: "NotFound" });
    if (isDocStatusLocked(existing.status)) {
      return reply.code(409).send({ error: "Locked", message: `Акт в статусе ${existing.status} нельзя удалить` });
    }
