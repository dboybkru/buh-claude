...
export async function actsRoutes(app: FastifyInstance) {
  ...
  app.post("/", async (request, reply) => {
    ...
    try {
      const created = await prisma.$transaction(async (tx) => {
        const number = data.number ?? (await nextDocumentNumber(tx, ownerUserId, data.organizationId, "ACT", year));
        ...
        await tx.documentItem.createMany({
          data: prepared.map((it) => itemCreateData(it, ownerUserId, "ACT", act.id)),
        });
        return act;
      });
  ...
  app.patch("/:id", async (request, reply) => {
    ...
    if (!parsed.success) return reply.code(400).send({ error: "ValidationError", details: parsed.error.flatten() });
    const userId = request.user.sub;
    const existing = await prisma.act.findFirst({ where: { id } });
    ...
    try {
      const updated = await prisma.$transaction(async (tx) => {
        ...
        if (data.items) {
          ...
          await tx.documentItem.createMany({ data: prepared.map((it) => itemCreateData(it, existing.userId, "ACT", id)) });
